 ...
 *
 * The entire data model lives under a single chrome.storage.local key ("links"):
 *
 *   {
...
(function (g) {
  ...
  async function gsGetLinks() {
    const data = await chrome.storage.local.get(GS_KEY);
    const links = data[GS_KEY];
    return links && typeof links === "object" ? links : {};
  }

  /** Overwrite the full alias -> config map. */
  async function gsSetLinks(links) {
    await chrome.storage.local.set({ [GS_KEY]: links });
  }
  ...
  async function gsGetTheme() {
    const data = await chrome.storage.local.get(GS_THEME_KEY);
    const t = data[GS_THEME_KEY];
    return t === "light" || t === "dark" || t === "system" ? t : "system";
  }

  async function gsSetTheme(theme) {
    await chrome.storage.local.set({ [GS_THEME_KEY]: theme });
  }
